...
│                                                                 │
│  하네스 훅 (자동, .claude/settings.local.json)                   │
│  ├── block-prod.sh     prod 브랜치 차단 + --no-verify 차단      │
│  ├── quality-gate.sh   커밋 전 TS/Rust 코드 검증                │
...
| 항목        | 저장 위치                                        |
| ----------- | ------------------------------------------------ |
| 스펙 소스   | Context Pack (사전 수집)                         |
| 대시보드    | 앱 내장 Dashboard                                |
| 상태 추적   | `task.pipeline` (localStorage)                   |
| dev-plan    | `task.pipeline.devPlan` (메모리 + localStorage)  |
| 스킬 소스   | `.claude/commands/pipeline/*.md` (프로젝트 로컬) |
| 대화 연속성 | Claude CLI `--resume {session_id}`               |
| Phase 전환  | 앱에서 직접 전환 + Claude 마커 보조              |
| 토큰 추적   | Phase별 input/output 토큰 + 비용                 |
| 알림        | macOS Notification                               |

외부 파일 저장(노트 앱, dev-plan.md 파일 등) 일체 사용 안 함.
...
| Save → Dev Plan      | Claude 마커 `[PIPELINE:save:done]`     |
...
| Git Hook (pre-commit)    | console.log, any 타입, 시크릿, Tauri 정적 import 검증 | `--no-verify`로만 우회        |
...
├── .claude/
│   ├── commands/pipeline/
│   │   ├── dev-task.md            ← Grill-me + 스펙 정리 스킬
│   │   ├── dev-implement.md       ← 개발 계획 + 구현 + PR 스킬
│   │   └── dev-resume.md          ← 중단 복구 스킬
│   ├── hooks/
│   │   ├── block-prod.sh          ← prod 브랜치 차단 + --no-verify 차단
│   │   ├── quality-gate.sh        ← 커밋 전 TS/Rust 코드 검증
│   │   └── confirm-delete.sh      ← 파일 삭제 확인
│   └── settings.local.json        ← 훅 설정 + 퍼미션
├── src/
│   ├── components/
