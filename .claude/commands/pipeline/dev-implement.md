...
- **grill-me 결과만으로 개발 계획을 작성**합니다. 재탐색·재검증을 통한 "보강"을 시도하지 마세요.
- Step 1 허용 도구: **Read 최대 2회** — grill-me에서 이미 지목된 파일 경로에 한함.
- Step 1 금지 도구: **Agent, Grep, Glob, Bash 탐색, WebFetch** — 예외 없음. "시그니처 확인"을 빌미로 Grep/Agent 소환 금지.
- **탐색 선언 문구 금지**. "코드베이스를 확인합니다", "기존 패턴을 파악합니다", "관련 코드를 분석합니다" 같은 문구 출력 자체 금지. 첫 출력은 반드시 계획서 템플릿.
- Read가 필요하면 선언 없이 바로 호출.
...
**첫 액션 규칙**: 이 마커 출력 직후 **바로 계획서 템플릿(## 개발 계획: ...)을 작성**합니다. 탐색·확인·분석 선언 문구는 한 줄도 출력하지 마세요. Read가 필요하면 선언 없이 바로 호출(최대 2회, grill-me 지목 파일에 한함). Agent·Grep·Glob 전면 금지.
...
- "코드베이스에서 기존 프록시 패턴과 Country 관련 코드를 확인합니다." → Agent 호출
- "관련 코드를 분석한 뒤 계획을 작성하겠습니다." → Grep/Glob 호출
- "구현 범위를 파악하기 위해 먼저 탐색합니다." → Agent 호출

위 문장을 출력하는 것 자체가 위반입니다. grill-me 결과가 이미 스펙이므로 추가 탐색이 불필요합니다.

✅ **올바른 첫 출력**: 시작 메시지 바로 뒤에 `## 개발 계획: {TASK_ID}` 템플릿으로 진입. 파일 시그니처가 정말 필요하면 선언 없이 Read 호출 후 템플릿 작성.
