...
**Plan Mode 에서는 계획을 채팅에 출력하지 말고 `ExitPlanMode` 도구로만 제출**하세요. 계획 본문 markdown 전체를 `plan` 필드에 넣어 호출합니다 — Cortx 가 해당 plan 으로 승인 카드를 렌더하므로 채팅에 같은 내용을 중복 출력하면 두 번 보이게 됩니다. 사용자에게 "진행할까요?" 라고 물어볼 필요도 없음 — 버튼으로 승인합니다. 세션은 ExitPlanMode 후 자동 종료되며, 승인 시 `/pipeline:_approve-plan` 로 재스폰됩니다.
