...
**스펙 입력**: 이 메시지 상단의 `## 📋 GRILL-ME 결과` 섹션이 완전한 개발 스펙입니다. Cortx가 이전 grill-me Q&A 전체를 자동 주입했습니다.

**첫 액션 규칙**: 이 마커 출력 직후, 탐색·확인·분석 선언 없이 **즉시 계획서 작성**:

- GRILL-ME 결과만으로 계획 초안 작성 가능 → 바로 계획서 템플릿(`## 개발 계획: ...`) 작성.
- 특정 파일 패턴 확인이 필요하면 → 선언 없이 **필요한 Read 를 한 응답 안에 batch** (병렬 Read tool_use 동시 emit). 순차 호출 금지.
- Agent/Grep/Glob 전체 스캔/Serena MCP 금지. Bash find/grep/rg/ls -R 금지.
