...
- **사용자가 질문하면 답변만 하고 멈추세요.** 답변 후 "다음 질문으로 넘어갈까요?" 라고 물어보고, 사용자가 허락한 후에만 다음 질문을 합니다. 절대 답변 직후 바로 다음 질문을 이어붙이지 마세요.
...
\`\`\`
✅ Grill-me 완료 & 개발 계획서 작성 완료

👉 다음 단계: /pipeline:dev-implement {TASK_ID}
\`\`\`
`,
...
🔗 PR: {PR URL}
👉 Copilot 리뷰가 달리면: /pipeline:dev-review-loop {TASK_ID}
\`\`\`
`,
